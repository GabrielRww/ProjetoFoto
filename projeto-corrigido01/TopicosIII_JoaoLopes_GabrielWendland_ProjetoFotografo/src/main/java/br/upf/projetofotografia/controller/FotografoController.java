...
public class FotografoController implements Serializable {
    ...
    public void salvar() {
        ...
        recarregarFotografos(); // Recarrega a lista após salvar
        entity = new FotografoEntity(); // Limpa o formulário
    }

    public void excluir() {
        if (selected != null) {
            facade.remove(selected);
            recarregarFotografos(); // Recarrega a lista após excluir
            selected = null;
        }
    }
    ...
    public void setSelected(FotografoEntity selected) {
        ...
        if (selected != null) {
            this.entity = selected; // Carrega os dados do fotógrafo selecionado para edição
        }
    }

    public List<FotografoEntity> getList() { return list; }

    private void recarregarFotografos() {
        list = facade.findAll();
    }
}
